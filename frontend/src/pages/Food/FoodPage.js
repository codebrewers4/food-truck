...
export default function FoodPage() {
  const [food, setFood] = useState(null); // change to null for correct initial state check
  const { id } = useParams();
    ...
    <div className={classes.container}>
      ...
      <div className={classes.details}>
        ...
        <div className={classes.stock}>
          <span>{food.stock > 0 ? `In stock: ${food.stock}` : 'Out of stock'}</span>
        </div>
        ...
        <button onClick={handleAddToCart} disabled={food.stock === 0}>Add To Cart</button>
      </div>
    </div>
  );
}
